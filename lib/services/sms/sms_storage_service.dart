...
class SmsStorageService {
  ...
  Future<List<Map<String, dynamic>>> listVisibleMessagesNeedingScreening({
    required int limit,
    int? beforeTimestampMs,
  }) async {
    await initialize();
    final effectiveLimit = limit <= 0 ? 5 : limit;
    final db = await _maybeOpenDatabase();
    if (db == null) {
      final rows = _memoryMessages.values
          .expand((List<Map<String, dynamic>> entries) => entries)
          .where((Map<String, dynamic> row) {
            final isOutgoing =
                row['isOutgoing'] == true || row['sender'] == 'Me';
            if (isOutgoing) {
              return false;
            }
            final timestampMs = _coerceInt(row['timestampMs']) ?? 0;
            if (beforeTimestampMs != null &&
                beforeTimestampMs > 0 &&
                timestampMs >= beforeTimestampMs) {
              return false;
            }
            final detectionDecision =
                row['detectionDecision']?.toString().trim() ?? '';
            final detectionSource =
                row['detectionSource']?.toString().trim() ?? '';
            return detectionDecision.isEmpty &&
                (detectionSource.isEmpty || detectionSource == 'provider_sync');
          })
          .map((Map<String, dynamic> row) => Map<String, dynamic>.from(row))
          .toList(growable: true)
        ..sort((a, b) {
          final aTs = _coerceInt(a['timestampMs']) ?? 0;
          final bTs = _coerceInt(b['timestampMs']) ?? 0;
          return bTs.compareTo(aTs);
        });
      if (rows.length > effectiveLimit) {
        rows.removeRange(effectiveLimit, rows.length);
      }
      return rows;
    }
    ...
    final whereParts = <String>[
      'is_outgoing = 0',
      '(detection_decision IS NULL OR detection_decision = \'\')',
      '(detection_source IS NULL OR detection_source = \'\' OR detection_source = \'provider_sync\')',
    ];
    final whereArgs = <Object>[];
    if (beforeTimestampMs != null && beforeTimestampMs > 0) {
      whereParts.add('timestamp_ms < ?');
      whereArgs.add(beforeTimestampMs);
    }
  ...
  String buildQuarantineId(SmsMessage message) {
    final timestampMs = message.time.millisecondsSinceEpoch;
    return message.messageKey?.trim().isNotEmpty == true
  ...
  Future<void> restoreQuarantineMessage(String quarantineId) async {
    ...
    final sender = entry['sender']?.toString() ?? '';
    final messageKey = entry['messageKey']?.toString();
    final localMessageId = messageKey?.trim().isNotEmpty == true
        ? 'local_${messageKey!.trim()}'
        : null;

    await removeVisibleMessage(
      peer: sender,
  ...
  Future<void> _recomputeThreadSummary(
    ...
    final latestVisibleNeedsScreening =
        !useQuarantine &&
        visibleLatest != null &&
        _isIncomingRowPendingScreening(visibleLatest);
    final latestMessage = useQuarantine
        ? quarantineLatest['message']?.toString() ?? ''
        : latestVisibleNeedsScreening
            ? 'Screening message...'
  ...
  Future<void> _rebuildMemoryThread(String threadId) async {
    ...
    final latestVisibleNeedsScreening =
        !useQuarantine &&
        latestVisible != null &&
        _isIncomingMessagePendingScreening(latestVisible);
  ...
  Map<String, Object?> _messageRowFromMap(
    ...
    final safetyStatus = _resolvedSafetyStatusForIncomingMap(map);
    return <String, Object?>{
  ...
  Map<String, Object?> _quarantineRowFromSmsMessage(
    ...
    final timestampMs = message.time.millisecondsSinceEpoch;
    final id = buildQuarantineId(message);
    return <String, Object?>{
      'id': id,
  ...
  Map<String, dynamic> _messageDisplayMapFromRow(Map<String, Object?> row) {
    final safetyStatus = _isIncomingRowPendingScreening(row)
        ? SafetyStatus.scanning.value
        : row['safety_status']?.toString() ?? 'safe';
    return <String, dynamic>{
  ...
  String _resolvedSafetyStatusForIncomingMap(Map<String, dynamic> map) {
    final explicit = map['safetyStatus']?.toString().trim() ?? '';
    if (explicit.isNotEmpty) {
      return explicit;
    }
    if (_isIncomingMessagePendingScreening(map)) {
      return SafetyStatus.scanning.value;
    }
    return map['isSuspicious'] == true ? 'malicious' : 'safe';
  }

  bool _isIncomingMessagePendingScreening(Map<String, dynamic> map) {
    final bool isOutgoing =
        map['isOutgoing'] == true || map['sender']?.toString() == 'Me';
    if (isOutgoing) {
      return false;
    }
    final detectionDecision = map['detectionDecision']?.toString().trim() ?? '';
    final detectionSource = map['detectionSource']?.toString().trim() ?? '';
    if (detectionDecision.isNotEmpty &&
        detectionDecision.toLowerCase() != 'scanning') {
      return false;
    }
    final safetyStatus = map['safetyStatus']?.toString().trim() ?? '';
    if (safetyStatus == SafetyStatus.malicious.value) {
      return false;
    }
    return detectionDecision.isEmpty ||
        detectionDecision.toLowerCase() == 'scanning' ||
        detectionSource.isEmpty ||
        detectionSource == 'provider_sync';
  }

  bool _isIncomingRowPendingScreening(Map<String, Object?> row) {
    final bool isOutgoing = (_coerceInt(row['is_outgoing']) ?? 0) == 1 ||
        row['sender']?.toString() == 'Me';
    if (isOutgoing) {
      return false;
    }
    final detectionDecision = row['detection_decision']?.toString().trim() ?? '';
    final detectionSource = row['detection_source']?.toString().trim() ?? '';
    if (detectionDecision.isNotEmpty &&
        detectionDecision.toLowerCase() != 'scanning') {
      return false;
    }
    final safetyStatus = row['safety_status']?.toString().trim() ?? '';
    if (safetyStatus == SafetyStatus.malicious.value) {
      return false;
    }
    return detectionDecision.isEmpty ||

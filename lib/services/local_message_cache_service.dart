...
class LocalMessageCacheService {
  ...
  static const String _dbPasswordKey = 'signal_cache_db_password_v1';
  ...
  Future<Database> _openDatabase() async {
    ...
    _databaseFuture = () async {
      ...
      final dbPath = p.join(directory.path, 'smishing_signal_cache.db');
  ...
  Future<void> _createSchema(Database db) async {
    ...
    await db.execute('''
      CREATE TABLE remote_identities (
        ...
        updated_at INTEGER NOT NULL,
        PRIMARY KEY (user_id, peer_uid, signal_device_id)
      )
    ''');
    await db.execute('''
  ...
  Future<Map<String, dynamic>?> readIdentity(String userId) async {
    final db = await _openDatabase();
  ...
  Future<Uint8List?> readRemoteIdentity({
    required String userId,
    required String peerUid,
    required int signalDeviceId,
  }) async {
    ...
    final rows = await db.query(
      ...
      where: 'user_id = ? AND peer_uid = ? AND signal_device_id = ?',
      whereArgs: [userId, peerUid, signalDeviceId],
      limit: 1,
    );
  ...
  Future<List<int>> readRemoteIdentityDeviceIds({
    ...
    final rows = await db.query(
      'remote_identities',
      columns: ['signal_device_id'],
      where: 'user_id = ? AND peer_uid = ?',
      whereArgs: [userId, peerUid],
    );
    return rows
        .map((row) => (row['signal_device_id'] as num?)?.toInt() ?? 1)
  ...
  Future<List<int>> getSessionDeviceIds({
    required String userId,
    required String peerUid,
  }) async {
    final db = await _openDatabase();
  ...
  Future<List<int>> getAllPreKeyIds(String userId) async {
    final db = await _openDatabase();
    final rows = await db.query(
      'prekeys',
      columns: ['prekey_id'],
      where: 'user_id = ?',
      whereArgs: [userId],
      orderBy: 'prekey_id ASC',
    );
    return rows
        .map((row) => (row['prekey_id'] as num?)?.toInt() ?? 0)
        .toList(growable: false);
  }

  Future<void> markPreKeyUploaded({
    required String userId,
    required int preKeyId,
  }) async {
    final db = await _openDatabase();
  ...
  Future<List<Map<String, dynamic>>> getPendingPreKeys(String userId) async {
    final db = await _openDatabase();
    return db.query(
      'prekeys',
  ...
  Future<Uint8List?> loadSignedPreKey({
    required String userId,
    required int preKeyId,
  }) async {
    final db = await _openDatabase();
  ...
  Future<List<int>> getAllSignedPreKeyIds(String userId) async {
    final db = await _openDatabase();
    final rows = await db.query(
      'signed_prekeys',
      columns: ['prekey_id'],
  ...
  Future<void> removeSignedPreKey({
    required String userId,
    required int preKeyId,
  }) async {
    final db = await _openDatabase();
  ...
  Future<Uint8List?> loadKyberPreKey({
    required String userId,
    required int preKeyId,
  }) async {
    final db = await _openDatabase();
  ...
  Future<List<int>> getAllKyberPreKeyIds(String userId) async {
    final db = await _openDatabase();
    final rows = await db.query(
      'kyber_prekeys',
      columns: ['prekey_id'],
  ...
  Future<void> markKyberPreKeyUploaded({
    required String userId,
    required int preKeyId,
  }) async {
    final db = await _openDatabase();
  ...
  Future<Map<String, dynamic>> exportSignalState(String userId) async {
    ...
    final remoteIdentities = await db.query(
      'remote_identities',
      where: 'user_id = ?',
      whereArgs: [userId],
    );
    ...
    final preKeys = await db.query(
      'prekeys',
      where: 'user_id = ?',
      whereArgs: [userId],
    );
    final signedPreKeys = await db.query(
      'signed_prekeys',
      where: 'user_id = ?',
      whereArgs: [userId],
    );
    final kyberPreKeys = await db.query(
      'kyber_prekeys',
      where: 'user_id = ?',
      whereArgs: [userId],
    );

    Map<String, dynamic> encodeRow(Map<String, Object?> row) {
    ...
    return {
      'identity': identityRows.isEmpty ? null : encodeRow(identityRows.first),
      'remoteIdentities': remoteIdentities.map(encodeRow).toList(),
      'sessions': sessions.map(encodeRow).toList(),
      'preKeys': preKeys.map(encodeRow).toList(),
      'signedPreKeys': signedPreKeys.map(encodeRow).toList(),
      'kyberPreKeys': kyberPreKeys.map(encodeRow).toList(),
    };
  }

  Future<void> importSignalState(
    String userId,
    ...
    await db.transaction((txn) async {
      await txn.delete('identities', where: 'user_id = ?', whereArgs: [userId]);
      await txn.delete(
        'remote_identities',
        where: 'user_id = ?',
        whereArgs: [userId],
      ...
      await txn.delete('prekeys', where: 'user_id = ?', whereArgs: [userId]);
      await txn.delete(
        'signed_prekeys',
        where: 'user_id = ?',
        whereArgs: [userId],
      ...
      if (identity is Map<String, dynamic>) {
        await txn.insert('identities', await decodeRow(identity));
      }
      for (final tableEntry in <String, String>{
        'remoteIdentities': 'remote_identities',
        'sessions': 'sessions',
        'preKeys': 'prekeys',
        'signedPreKeys': 'signed_prekeys',
        'kyberPreKeys': 'kyber_prekeys',
      }.entries) {
        final rows = payload[tableEntry.key] as List<dynamic>? ?? const [];
        for (final row in rows) {
          if (row is! Map<String, dynamic>) continue;
          await txn.insert(tableEntry.value, await decodeRow(row));
        }
      }
    });
  }

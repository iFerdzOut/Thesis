...
class LocalDetectionRepository {
  ...
  final Map<String, String> _memorySettings = <String, String>{
    _singleThresholdKey: '0.50',
    _warningThresholdKey: '0.50',
    _quarantineThresholdKey: '0.50',
  };

  Future<void> initialize() async {
    final db = await _maybeOpenDatabase();
    await _ensureThresholdSeeds(db);
  ...
  Future<void> _ensureThresholdSeeds(Database? db) async {
    // If the DB couldn't be opened, the in-memory defaults already apply.
    if (db == null) {
      _memorySettings.putIfAbsent(_singleThresholdKey, () => '0.50');
      _memorySettings.putIfAbsent(_warningThresholdKey, () => '0.50');
      _memorySettings.putIfAbsent(_quarantineThresholdKey, () => '0.50');
      return;
    }

    final single = await getSetting(_singleThresholdKey);
    final warning = await getSetting(_warningThresholdKey);
    final quarantine = await getSetting(_quarantineThresholdKey);

    if (warning == null || warning.trim().isEmpty) {
      await setSetting(
        key: _warningThresholdKey,
        value: (single != null && single.trim().isNotEmpty)
            ? single.trim()
            : '0.50',
      );
    }
    if (quarantine == null || quarantine.trim().isEmpty) {
      await setSetting(
        key: _quarantineThresholdKey,
        value: (single != null && single.trim().isNotEmpty)
            ? single.trim()
    ...
    if (single == null || single.trim().isEmpty) {
      await setSetting(key: _singleThresholdKey, value: '0.50');
    }

    final normalizedSingle =
        (await getSetting(_singleThresholdKey))?.trim() ?? '0.50';
    final normalizedWarning = (await getSetting(_warningThresholdKey))?.trim();
    final normalizedQuarantine =
        (await getSetting(_quarantineThresholdKey))?.trim();
    if (normalizedWarning != '0.50' || normalizedQuarantine != '0.50') {
      await setThresholds(
        warningThreshold: 0.50,
        quarantineThreshold: 0.50,
      );
      if (normalizedSingle != '0.50') {
        await setSetting(key: _singleThresholdKey, value: '0.50');
      }
    }
  }
  ...
  Future<void> saveScreeningResult({
    ...
    await db.insert(
      ...
      <String, Object?>{
        ...
        'warning_threshold': result.quarantineThreshold,
